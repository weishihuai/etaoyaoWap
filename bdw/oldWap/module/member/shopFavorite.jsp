...
<body>
<%--页头开始--%>
<c:import url="/template/bdw/oldWap/module/common/head.jsp?title=店铺收藏"/>
<%--页头结束--%>
<div class="container">
    <c:set value="${userShopCollectPage.result}" var="shopProxy"/>
    <c:choose>
        <c:when test="${not empty shopProxy}">
            <c:forEach items="${userShopCollectPage.result}" var="shop" varStatus="statu">
                <c:if test="${not empty shop}">
                    <div class="row list_rows"style="padding-bottom: 0px;">
                        <div class="col-xs-4">
                            <div class="list_pic">
                                <c:choose>
                                    <c:when test="${not empty shop.shopType && shop.shopType == '2'}">
                                        <a href="javascript:" orgId="${shop.sysOrgId}" isSupportBuy="${shop.isSupportBuy}" onclick="gotoStore(this);"><img alt="${shop.shopNm}" src="${webRoot}/upload/${shop.shopPicUrl}" width="80" height="80"></a>
                                    </c:when>
                                    <c:otherwise>
                                        <a href="${webRoot}/wap/module/shop/index.ac?shopId=${shop.shopInfId}"><img alt="${shop.shopNm}" src="${webRoot}/upload/${shop.shopPicUrl}" width="80" height="80"></a>
                                    </c:otherwise>
                                </c:choose>
                        ...
                        <div class="col-xs-8">
                            <div class="list_title">
                                <c:choose>
                                    <c:when test="${not empty shop.shopType && shop.shopType == '2'}">
                                        <a href="javascript:" orgId="${shop.sysOrgId}" isSupportBuy="${shop.isSupportBuy}" onclick="gotoStore(this);">${shop.shopNm}<em></em></a>
                                    </c:when>
                                    <c:otherwise>
                                        <a href="${webRoot}/wap/module/shop/index.ac?shopId=${shop.shopInfId}">${shop.shopNm}<em></em></a>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                            <div class="online-time">
                                <span>在线时间：${shop.csadOnlineDescr}</span><br/>
                            </div>
                            <div class="delete-icon">
                                <span class="glyphicon glyphicon-trash pull-right delItem" itemkey="04112db0-6c85-4750-bc8e-29d6be583e4e" handler="sku" carttype="normal" style="color:#999;padding-left: 20px;padding-right: 20px; line-height: 22px;" onclick="doShopCollectDelete(${shop.shopInfId});"></span>
                            </div>
                        </div>
                    </div>
                </c:if>
    ...
    <div class="pn-page row">
        <form action="${webRoot}/module/member/shopCollection.ac" id="pageForm" method="post" style="display: inline;" totalPages='${userProductPage.lastPageNumber}' currentPage='${_page}' totalRecords='${userProductPage.totalCount}' frontPath='${webRoot}' displayNum='5'>
