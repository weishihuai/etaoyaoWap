...
!function(w, $){
	...
	Factory.prototype = {
		...
		init: function(){
			...
			c.html(
				...
				+	(opt.type ? '<div style="text-align:center;"><span style="background:url('+ utils.getPath + "/"+opt.skinPath + opt.type +'_icon.gif) no-repeat 0 center;display:inline-block;padding:2.05rem 0 0.75rem 3.625rem;">' + opt.content + '</span></div>' : opt.content)
				+ '</div>'
					/*底部区域*/
	...
	var interface = (function(){
		...
		/*
		 ===简单信息弹窗(可接收四个参数[可向左容错])===
		 1. 参数1个 信息内容
		 2. 参数2个 {a:1~2组合[信息内容~配置参数] | b:1~3组合[信息内容~执行函数]}
		 3. 参数3个 信息内容~配置参数~执行函数
		 4. 参数4个 自定关闭的时间
		 */
		exports.msg = function(content, options, fn, time){
			Factory(
				$.extend({
					title: false,
					content: content,
	...
	//加载css文件
	interface.load(utils.getPath+"skin/xyPop.css");
