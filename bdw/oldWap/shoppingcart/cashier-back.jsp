...
<form action="${webRoot}/cashier/goBank.ac" method="post" id="goBank" <%--onsubmit="return submitGoBank()"--%>>
    ...
    <div class="col-xs-12 ck-title" >合并<i>${orderSize}</i>笔订单<a href="#"  id="showOrder" ></a></div>
        <c:forEach items="${orderList}" var="order">
            <div class="row order-one showMyOrder hideThis">
                ...
                <div class="col-xs-12 form">金额:${order.orderTotalAmount}</div>
            </div>
        </c:forEach>
...
<%--    <div class="col-xs-12 ano-pay">使用余额支付<em>￥0.00</em>，剩下金额用其他方式付款</div>--%>
    ...
    <c:if test="${empty param.payWayId}">
    <div class="col-xs-12 payment"><i>选择支付方式：</i>
        ...
        <%--<c:if test="${not empty param.payWayId&&param.payWayId!=102}">   </c:if>--%>
