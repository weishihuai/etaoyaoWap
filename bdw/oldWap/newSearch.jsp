...
<head>
    ...
    <script src="${webRoot}/template/bdw/oldWap/statics/js/newSearch.js"></script>
        ...
        <c:forEach items="${hotKeywords}" var="hotKeyword" end="9">
            <c:choose>
                <c:when test="${not empty hotKeyword}">
                   <a href="${webRoot}/wap/list.ac?keyword=${hotKeyword}"><dd role="button"  title="${hotKeyword}">${hotKeyword}</dd></a>
                </c:when>
            </c:choose>
        </c:forEach>
    </dl>
