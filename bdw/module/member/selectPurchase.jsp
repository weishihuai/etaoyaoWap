...
<c:set value="${sdk:findReturnOrderPage(loginUser.userId,pageNum,8,true)}" var="orderProxyPage"/> <%--获取当前用户普通订单--%>
                                             ...
                                             <li>
                                                 <div class="returnItem">
                                                     <c:choose>
                                                         ...
                                                         <c:otherwise>
                                                             <input type="checkbox" value="${orderItemProxy.orderItemId}" name="${orderProxy.orderId}" <c:if test="${orderItemProxy.isCanReturn == false }">disabled</c:if> />
                                                             <a href="${webRoot}/product-${orderItemProxy.productProxy.productId}.html" title="${orderItemProxy.productProxy.name}"><img src="${orderItemProxy.productProxy.defaultImage["50X50"]}" width="48px;" height="48px;" alt="${orderItemProxy.productProxy.name}"/></a>
                                                                             <span>
