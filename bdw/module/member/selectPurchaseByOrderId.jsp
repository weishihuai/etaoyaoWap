...
<c:set value="${sdk:findOrderDetailed(orderId)}" var="orderProxy"/> <%--获取当前用户普通订单--%>
...
<head>
    ...
    <title>${webName}-选择退换货-${sdk:getSysParamValue('index_title')}</title><%--SEO title优化--%>
   ...
   <div class="rBox">
       <div class="seachOb">
	      <div class="m1">
              ...
              <h3>选择退货商品<span style="font-size:12px;font-weight:normal;"> -- 你在${returned_date}天内购买的商品享受退货服务</span><div class="btn" style="float: right;"><a href="${webRoot}/module/member/orderList.ac?pitchOnRow=11">返回</a></div></h3>
              <div class="clear"></div>
			 <div class="box return">
                 ...
                 <table width="100%" border="0" cellspacing="0">
                     ...
                     <tr <c:if test="${status.count mod 2 !=0}"> class="bg"</c:if>>
                                         ...
                                         <li>
                                             <div class="returnItem">
                                                 <c:choose>
                                                     ...
                                                     <c:when test="${orderItemProxy.isCanReturn == true && orderItemProxy.combinedProductId != null}">
                                                         <span style="float: left;margin-left:30px;">&nbsp;</span>
                                                         <a href="${webRoot}/product-${orderItemProxy.productProxy.productId}.html" title="${orderItemProxy.productProxy.name}"><img src="${orderItemProxy.productProxy.defaultImage["50X50"]}" width="48px;" height="48px;" alt="${orderItemProxy.productProxy.name}"/></a>
                                                         ${fn:substring(orderItemProxy.productProxy.name,0,15)}(<span class="price">套餐暂不支持退货</span>)
                                                     </c:when>
                                                     <c:otherwise>
                                                         <input type="checkbox" value="${orderItemProxy.orderItemId}" name="${orderProxy.orderId}" <c:if test="${orderItemProxy.combinedProductId != null}">combinedProductId="${orderItemProxy.combinedProductId}"</c:if> <c:if test="${orderItemProxy.isCanReturn == false }">disabled</c:if> />
                                                         <a href="${webRoot}/product-${orderItemProxy.productProxy.productId}.html" title="${orderItemProxy.productProxy.name}"><img src="${orderItemProxy.productProxy.defaultImage["50X50"]}" width="48px;" height="48px;" alt="${orderItemProxy.productProxy.name}"/></a>
                                                         <span>
                                                                                 ${fn:substring(orderItemProxy.productProxy.name,0,8)}(<span class="price">¥${orderItemProxy.productUnitPrice}</span>) X ${orderItemProxy.canReturnNum}
                                                                             </span>
                                                     </c:otherwise>
                                                 </c:choose>
                                             </div>
                         ...
                         <td class="td7">
                             <div class="btn"><a href="javascript:" onclick="selectPurchase('${orderProxy.orderId}','${orderProxy.orderNum}',true);">退货</a></div>
                         </td>
                     </tr>
                 </table>
                 <%--订单显示列表 end--%>
