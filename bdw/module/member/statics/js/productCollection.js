...
function reomveCollection($Arr){
    $.ajaxSettings['contentType'] = "application/x-www-form-urlencoded; charset=utf-8";
    var cancelCollection = jDialog.confirm('<span style="margin-left: 10px">确定取消收藏该商品吗!</span>',{
        type : 'highlight',
        text : '确定',
        handler : function(button,cancelCollection) {
            cancelCollection.close();
            $.ajax({
                type:"POST",url:dataValue.webRoot+"/member/delUserProductCollect.json",
                data:{items:$Arr.join(",")},
                dataType:"json",
                success:function(data){
                    if (data.success == "true") {
                        ...
                        productCollectionAlertDialog("系统错误,请刷新重新操作!");
                    }
                }
            });
        }
...
var productCollectionAlertDialog = function(dialogTxt){
