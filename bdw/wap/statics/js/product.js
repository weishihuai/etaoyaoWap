    ...
    $("#cpsShare").click(function(){
        if(webPath.isWeixin=='Y') {//微信端显示引导层
            ...
            if($("#loadDiv").html().trim()==''){
                ...
                var $shareUrl = wapUrl +"/wap/product-"+$shareId+".html?1=1";
                $("#loadDiv").load(wapUrl +"/wap/cpsWapExtension.ac?shareId="+$shareId+"&from=product"+"&shareUrl="+$shareUrl);
            }else{
            ...
            if (data.success == "true") {
                if(isBuyNow){
                    if(carttype == 'normal'){//普通购物车
                        window.location.href = webPath.webRoot+ "/wap/shoppingcart/cart.ac?pIndex=cart";
                    }else{
                        window.location.href = webPath.webRoot+ "/wap/shoppingcart/drugCart.ac?carttype=drug&handler=drug&pIndex=cart";
                    }
                }else{
