...
<head lang="en">
    ...
    <script src="${webRoot}/template/bdw/wap/statics/js/shopProductList.js" type="text/javascript"></script>
    <style type="text/css">
        .list-main .list-head-b .sp-sort > li{
            width: 33.33%;
        }
    </style>
</head>
        ...
        <div class="list-head-b">
            <ul class="sp-sort clearfix">
                ...
                <c:if test="${param.order == 'minPrice,asc' or param.order == 'minPrice,desc'}"><div class="dt">价格</div></c:if>
                    <ul class="dd">
                        <li><a data-active="${param.order==""||param.order==null?'true':'false'}" href="${webRoot}/wap/shop/shopProductList.ac?shopId=${shopId}&shopCategoryId=${shopCategoryId}&q=${param.q}&keyword=${param.keyword}&isInStore=${param.isInStore}&page=1">综合</a></li>
                        <li><a data-active="${param.order=='minPrice,asc'?'true':'false'}" href="${webRoot}/wap/shop/shopProductList.ac?shopId=${shopId}&shopCategoryId=${shopCategoryId}&q=${param.q}&order=minPrice,asc&keyword=${param.keyword}&isInStore=${param.isInStore}&page=1">价格从低到高</a></li>
                        <li><a data-active="${param.order=='minPrice,desc'?'true':'false'}" href="${webRoot}/wap/shop/shopProductList.ac?shopId=${shopId}&shopCategoryId=${shopCategoryId}&q=${param.q}&order=minPrice,desc&keyword=${param.keyword}&isInStore=${param.isInStore}&page=1">价格从高到低</a></li>
                    </ul>
                </li>
                <c:if test="${param.order=='salesVolume,desc'}">
                    <li class="cur high-to-low"><a href="${webRoot}/wap/shop/shopProductList.ac?shopId=${shopId}&shopCategoryId=${shopCategoryId}&q=${param.q}&keyword=${param.keyword}&page=1&isInStore=${param.isInStore}&order=salesVolume,asc"><span style="color: #FF6B00;">销量</span></a></li>
                </c:if>
                <c:if test="${param.order=='salesVolume,asc'}">
                    <li class="cur"><a href="${webRoot}/wap/shop/shopProductList.ac?shopId=${shopId}&shopCategoryId=${shopCategoryId}&q=${param.q}&keyword=${param.keyword}&page=1&isInStore=${param.isInStore}&order=salesVolume,desc"><span style="color: #FF6B00;">销量</span></a></li>
                </c:if>
                <c:if test="${param.order !='salesVolume,desc' and param.order !='salesVolume,asc'}">
                    <li><a href="${webRoot}/wap/shop/shopProductList.ac?shopId=${shopId}&shopCategoryId=${shopCategoryId}&q=${param.q}&keyword=${param.keyword}&page=1&isInStore=${param.isInStore}&order=salesVolume,asc"><span>销量</span></a></li>
                </c:if>
...
<%--下拉加载更多--%>
<nav id="page-nav">
    <a href="${webRoot}/wap/shop/loadMoreShopProductList.ac?shopId=${shopId}&order=${param.order}&shopCategoryId=${shopCategoryId}&q=${param.q}&keyword=${param.keyword}&startPrice=${param.startPrice}&endPrice=${param.endPrice}&isInStore=${param.isInStore}&page=2&limit=${limit}"></a>
</nav>
