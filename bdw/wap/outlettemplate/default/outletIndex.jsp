...
<c:choose>
    ...
    <c:otherwise>
        <c:set var="imgUrl" value="${shop.images[0]['']}"/>
    </c:otherwise>
</c:choose>
...
<div class="m-top">
   ...
   <%-- <div class="store-more">
        ...
        <div style="display: none;" class="more-box">
            ...
            <a href="${webRoot}/wap/outlettemplate/default/outletIndex.ac?shopId=${shop.shopInfId}" class="home-page">首页</a>
...
<header class="header-wrapper">
    <div class="sc-box1">
        ...
        <div class="search-form-box" onclick="window.location.href='${webRoot}/wap/outlettemplate/default/newSearch.ac?shopId=${shop.shopInfId}'">
            ...
            <c:when test="${not empty topicLinks}">
                <c:forEach items="${topicLinks}" var="links" end="3">
                    <a <c:if test="${links.newWin}">target="_blank" </c:if> href="${links.link}" class="quick-entry-link">
                        <span class="pic"><img src="${links.icon['100X100']}"></span>
                        <i>${links.title}</i>
                    </a>
                </c:forEach>
            </c:when>
            <c:otherwise>
                <a href="${webRoot}/wap/outlettemplate/default/productList.ac?shopId=${shopId}" class="quick-entry-link">
                    <span class="pic"><img src="${webRoot}/template/bdw/wap/statics/images/pics.png" alt=""></span>
                    <i>所有商品</i>
                </a>
                <a href="${webRoot}/wap/outlettemplate/default/storeDetail.ac?shopId=${param.shopId}" class="quick-entry-link">
                    <span class="pic"><img src="${webRoot}/template/bdw/wap/statics/images/picx.png" alt=""></span>
                    <i>店铺详情</i>
                </a>
                <a href="${webRoot}/wap/outlettemplate/default/newSearch.ac?shopId=${shop.shopInfId}" class="quick-entry-link">
                    <span class="pic"><img src="${webRoot}/template/bdw/wap/statics/images/picg.png" alt=""></span>
                    <i>店铺代购</i>
                </a>
                <a href="${webRoot}/wap/outlettemplate/default/promotionList.ac?shopId=${param.shopId}" class="quick-entry-link">
                ...
                <c:choose>
                    <c:when test="${not empty productRecommends}">
                        <c:forEach items="${productRecommends}" var="product">
                            <li class="rec-li"><a href="${webRoot}/wap/outlettemplate/default/product.ac?id=${product.productId}">
...
<div class="store-nav">
    <div class="str-detail"><a href="${webRoot}/wap/outlettemplate/default/outletIndex.ac?shopId=${param.shopId}">店铺首页</a></div>
    <div class="str-classify">
        <c:set var="categoryEmpty" value="${empty shopCategory}"/>
        <c:choose>
            <c:when test="${categoryEmpty}">
                <a href="${webRoot}/wap/outlettemplate/default/productList.ac?shopId=${shopId}" class="classify">商品分类</a>
            </c:when>
            <c:otherwise>
                <a href="javascript:;" class="classify">商品分类</a>
            </c:otherwise>
        </c:choose>

            <div style="display: none;" class="csf-box">
                <c:if test="${not categoryEmpty}">
                    <c:forEach items="${shopCategory}" var="node" varStatus="s" end="4">
                        <a href="${webRoot}/wap/outlettemplate/default/productList.ac?category=${node.categoryId}&shopId=${shopId}">${node.name}</a>
                    </c:forEach>
                </c:if>
            </div>
    </div>
    <div class="str-detail str-cat"><a href="${webRoot}/wap/outlettemplate/default/shoppingcart/drugCart.ac?carttype=store_drug&handler=store_drug">购物车</a></div>
