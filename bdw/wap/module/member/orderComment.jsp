...
<%--判断是否允许评论--%>
<c:set value="${sdk:wapIsAllowComment(param.id)}" var="allowComment"/>
<c:if test="${!allowComment}">
    <c:redirect url="${webRoot}/wap/module/member/orderCommentSuccess.ac?id=${param.id}"></c:redirect>
</c:if>
<c:set value="${orderProxy.orderComment}" var="orderComment"/>
<%--&lt;%&ndash;判断是否已评论&ndash;%&gt;--%>
<%--<c:if test="${orderComment.isCommentFinish == 'Y'}">--%>
<%--<c:redirect url="${webRoot}/wap/module/member/orderCommentSuccess.ac?id=${param.id}"></c:redirect>--%>
<%--</c:if>--%>
<html>
<head>
    ...
    <script src="${webRoot}/template/bdw/wap/module/member/statics/js/orderComment.js"></script>
    ...
    <script type="text/javascript">
        var webPath = {
            ...
            isWeixin:"${isWeixin}",
            orderId: '${param.id}',
            orderItemsNum:'${fn:length(orderProxy.orderItemProxyList)}',
            status:'${param.status}',
        };
    </script>
</head>
<body>
    ...
    <div class="appraisel-main">
        ...
        <c:forEach items="${orderProxy.orderItemProxyList}" var="orderItem" varStatus="s">
            ...
            <c:set var="contentLength" value="${fn:length(comment.content)}"/>
            ...
            <div class="product-appraisel">
                ...
                <div class="dd">
                    ...
                    <div class="appraisel-txt">
                        <textarea class="addCommentTextArea" id="addCommentTextArea_${s.index}" placeholder="评价超过10字有机会获得积分～" >${comment.content}</textarea>
                        <span class="txt-number">  ${contentLength > 0 ? contentLength:'0'}/250</span>
                    </div>
                    <div class="add-pic-box clearfix">
                        <c:forEach items="${comment.pics}" var="image">
                            <div class="pic-item" style="z-index: 2" onclick='removePic(this)' >
                                <img src="${image['100X100']}" alt="">
                            </div>
                        </c:forEach>

                    <input type="hidden" id="commentPictList_${s.index}" name="commentPictList_${s.index}" value="${comment.pics}">
                            <a class="add-pic-btn" id="add-pic-btn_${s.index}"href="javascript:;"></a>
                    </div>
                </div>
            </div>
        </c:forEach>
        ...
        <a class="appraisel-btn"  id="commentSubmtBtn" href="javascript:void(0);" onclick="submitComment(this)">发表评价</a>
    </div>
